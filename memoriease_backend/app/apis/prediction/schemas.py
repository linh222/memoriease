...
class FeatureModelSingleSearch(BaseModel):
    ...
    topic_id: Optional[str] = Field(default='', description='Topic ID for NTCIR')
    run_id: Optional[str] = Field(default='', description='Run ID for NTCIR')
    ...
    def __str__(self):
        return f"{self.query}, {self.semantic_name}, {self.topic_id}, {self.run_id}"
...
class FeatureModelTemporalSearch(BaseModel):
    ...
    def __str__(self):
        return f"{self.query}," \
               ...
               f"{self.time_gap}, {self.semantic_name}, {self.topic_id}, {self.run_id}"
